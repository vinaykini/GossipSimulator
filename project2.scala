  ...
  def receive = {
    ...
    case (done: String) => {
      ...
      if (count == numNodes) {
        ...
        println("Node and corresponding message count: ")
        /*for (i <- 0 to numNodes - 1)
          println("I:" + i + " count:" + global.ind_count(i));*/
  ...
  def receive = {
    case (rec: String) =>
      {
        if (count < 10) {
          ...
          // println("Sending from " + n + " to " + neighbour + " with Random:" + random + " Length:" + len);
    ...
    case (sum: Double, weight: Double) => {
      if (flag == 0) {
        ...
        s = s / 2;
        w = w / 2;
        //var index: Int = pushCount % 3;
        //pushCount += 1;
        //pushArray(index) = ratio;
	
	slidingWindow.enqueue(ratio)
	
        //=============================================================================================
        //if (((Math.abs(pushArray(0) - pushArray(1))) <= 0.001) && ((Math.abs(pushArray(0) - pushArray(2))) <= 0.001)) {
	//println((Math.abs(slidingWindow.head - slidingWindow.last)))
	if ((Math.abs(slidingWindow.head - slidingWindow.last)) <= 0.001){
          //println("In new check!")
          println(n + " is done!")
          //println("ratio is " + ratio)
	  //println("And arrays are "+pushArray(0)+" "+pushArray(1)+" "+pushArray(2))
          flag = 1;
          //println("sending done to master Node::" + n + " ratio::" + ratio)
	...
	slidingWindow.dequeue
        ...
        //println("Sending from " + n + " to " + neighbour1 + " with random:" + random1 + " length:" + len+" sum and weight "+s+" "+w);
